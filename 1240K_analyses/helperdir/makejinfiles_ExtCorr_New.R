...
for(i in 1:22){
score=read.table(paste(args1,"_",args3,"panel_1ind_noChr",i,"_topscore_fullreads.txt",sep=""),header=F)
jinfile[i,3]=score[1,1]
}
num=mean(jinfile[,3])
writejin <- file(paste(args1,"_",args3,"panel_scores_reads_ExtCorr.jin",sep=""),"w")
write.table(jinfile,writejin,sep="\t",quote=FALSE,col.names=FALSE,row.names=FALSE)
flush(writejin)
close(writejin)
writenum <- file(paste(args1,"_",args3,"panel_scores_reads_ExtCorr_meanscore.txt",sep=""),"w")
