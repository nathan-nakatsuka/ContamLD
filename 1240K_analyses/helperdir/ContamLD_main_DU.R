...
SNPfreq_panel=read.table(paste(args[1],"/panels/",args[4],"_snpfreqs_heldout_noX.txt",sep=""),header=T,stringsAsFactors=F)
### These are the positions used to make the SNPfreq_panel and HaplotypeFreq_panel files.
SNPfreq_panel_snpfile=read.table(paste(args[1],"/panels/",args[4],"_eig_heldout_noX_ChromPos.snp",sep=""),header=F,stringsAsFactors=F)
...
### Shrink the snpfiles down to the places where there exists a SNP in the SNPfreq file.
snpfile_UU=snpfile_UU[(snpfile[,5] %in% SNPfreq_panel_snpfile[,1]),]
...
LDfile = read.table(paste(args[1],"/panels/",args[4],"_eig_heldout_noX.ld",sep=""),header=T)
...
HaplotypeFreq_panel=read.table(paste(args[1],"/panels/",args[4],"_haplotypefreqs_heldout_noX.txt",sep=""),header=T,stringsAsFactors=F,colClasses="character")
...
odd_UU=snpfile_UU[new[,1],]
odd2_UU=snpfile_UU[new[,2],]


Haplotypes_testindiv=data.frame(1:nrow(new))
## 1st column = # of 00 haplotypes that are DU or UD
Haplotypes_testindiv[,1]=odd[,4]*odd2_UU[,4]+odd_UU[,4]*odd2[,4]
## 2nd column = # of 02 haplotypes that are DU or UD
Haplotypes_testindiv[,2]=odd[,4]*odd2_UU[,3]+odd_UU[,4]*odd2[,3]
## 3rd column = # of 20 haplotypes that are DU or UD
Haplotypes_testindiv[,3]=odd[,3]*odd2_UU[,4]+odd_UU[,3]*odd2[,4]
## 4th column = # of 22 haplotypes that are DU or UD
Haplotypes_testindiv[,4]=odd[,3]*odd2_UU[,3]+odd_UU[,3]*odd2[,3]
...
Table2=data.frame(1:1)

## DamageRatio fill
temp1=snpfile_UU[snpfile_UU[,1],]
temp2=snpfile[snpfile[,1],]
### Divide all damaged reads by sum of undamaged and damaged reads
Table2[1,1]=1-(sum(temp2[,3])+sum(temp2[,4]))/(sum(temp1[,3])+sum(temp1[,4])+sum(temp2[,3])+sum(temp2[,4]))
write.table(Table2,file=paste(args[1],"_damageratio.txt",sep=""),sep="\t",quote=FALSE,col.names=FALSE,row.names=FALSE)

## Table2 for damage ratio
Table2=data.frame(0:22)
Table2[,2]=0
## DamageRatio fill
temp1=snpfile_UU[snpfile_UU[,1]!=0,]
temp2=snpfile[snpfile[,1]!=0,]
### Divide all damaged reads by sum of undamaged and damaged reads
Table2[(0+1),2]=1-(sum(temp2[,3])+sum(temp2[,4]))/(sum(temp1[,3])+sum(temp1[,4])+sum(temp2[,3])+sum(temp2[,4]))
...
## DamageRatio fill
temp1=snpfile_UU[snpfile_UU[,1]!=f,]
temp2=snpfile[snpfile[,1]!=f,]
### Divide all damaged reads by sum of undamaged and damaged reads
